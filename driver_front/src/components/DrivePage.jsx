...
const DrivePage = ({
    ...
    // video 스타일 - CSS로 표시/숨김 (video는 항상 DOM에 존재)
    const videoStyle = {
        position: showCameraView ? 'absolute' : 'fixed',
    ...
    // 메인 뷰 (showCameraView=false) - 홈 화면은 하얀 배경
    return (
        <div className="min-h-full relative bg-white">
            {/* 단일 video 요소 - 숨김 상태로 DOM에 유지 (srcObject 보존) */}
            <video
                ref={videoRef}
                autoPlay
                playsInline
